...
class UserManagementService {
  ...
  static Future<UserModel?> loadUserModel(String uid) async {
    try {
      ...
      if (doc.exists) {
        ...
        debugPrint('📊 Attempting to create UserModel with data: ${processedData.keys}');
  ...
  static Future<UserModel?> checkAndUpdateProfileCompletion(UserModel userModel) async {
    try {
      debugPrint('🔄 Checking profile completion for: ${userModel.email}');
      debugPrint('   Current isProfileComplete: ${userModel.isProfileComplete}');
      ...
      if (shouldBeComplete && !userModel.isProfileComplete) {
        debugPrint('✏️ Updating profile completion status for existing user: ${userModel.userId}');
        ...
        debugPrint('❌ Profile does not have essential data - keeping incomplete');
      }
  ...
  static bool hasEssentialProfileData(UserModel userModel) {
    ...
    bool hasBasicInfo = userModel.userName.isNotEmpty && 
                       userModel.email.isNotEmpty;
    
    bool hasUserType = userModel.userType.isNotEmpty && 
                      userModel.userType != 'unknown' &&
                      userModel.userType != '' &&
                      userModel.userType != 'pending';
    
    bool hasRole = userModel.currentRole.isNotEmpty;
    
    debugPrint('   Basic info: $hasBasicInfo (name: "${userModel.userName}", email: "${userModel.email}")');
    debugPrint('   User type: $hasUserType ("${userModel.userType}")');
    debugPrint('   Role: $hasRole ("${userModel.currentRole}")');
  ...
  static Future<bool> createUserDocumentFromExistingAuth(User user) async {
    try {
      debugPrint('🏗️ Creating user document for existing auth user: ${user.email}');
